  ...
  function AuthLinks({ status, userName }) {
    return status === "authenticated" ? (
      <>
        <Link className="hover:underline text-4xl  md:text-base lg:text-xl font-semibold" href="/profile">
          {userName}
        </Link>
        <button
      ...
      >
        <div className="container mx-auto flex justify-between items-center py-6 lg:py-7">
          ...
          <div className="md:hidden flex items-center gap-6">
            ...
            <button className="border-none" onClick={() => setMobileNavOpen(!mobileNavOpen)}>
              <Bars2 />
            </button>
          </div>
        </div>
        ...
        {/* Mobile Navigation */}
        <div
          className={`fixed z-30 top-0 left-0 w-full h-full bg-red-600 border-r border-gray-900 dark:border-gray-600 ease-in-out duration-500 md:hidden transform ${
            mobileNavOpen ? "translate-x-0" : "-translate-x-full"
          }`}
        >
          <div className="font-bold flex justify-end pt-5 text-5xl">
            <button
